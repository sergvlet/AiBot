...
import com.chicu.aibot.exchange.model.OrderInfo; // предположим, клиент возвращает OrderInfo
...
public class BinanceOrderMapper implements OrderResponseMapper {
    ...
    public OrderResponse map(Object rawResp) {
        if (!(rawResp instanceof OrderInfo resp)) {
            throw new IllegalArgumentException("Ожидался OrderInfo от Binance");
        }
